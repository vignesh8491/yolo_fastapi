...
#@torch.jit.ignore
def yolo_forward_dynamic(output, conf_thresh:float, num_classes:int, anchors:List[float], num_anchors:int, scale_x_y:int, only_objectness:int=1,
        validation:bool=False) -> List[torch.Tensor]:
    # Output would be invalid if it does not satisfy this assert
    # assert (output.size(1) == (5 + num_classes) * num_anchors)
    ...
    anchor_w = torch.zeros(num_anchors)
    anchor_h = torch.zeros(num_anchors)
    
    for i in range(num_anchors):
        anchor_w[i] = anchors[i*2]
        anchor_h[i] = anchors[i * 2 + 1]
    ...
    bx_list = torch.zeros((output.size(0), num_anchors, grid_x.size(0), grid_x.size(1), grid_x.size(2), grid_x.size(3)))
    by_list = torch.zeros((output.size(0), num_anchors, grid_x.size(0), grid_x.size(1), grid_x.size(2), grid_x.size(3)))
    bw_list = torch.zeros((output.size(0), num_anchors, grid_x.size(0), grid_x.size(1), grid_x.size(2), grid_x.size(3)))
    bh_list = torch.zeros((output.size(0), num_anchors, grid_x.size(0), grid_x.size(1), grid_x.size(2), grid_x.size(3)))
...
class YoloLayer(nn.Module):
    ...
    def forward(self, output)->List[torch.Tensor]:
        ...
        return yolo_forward_dynamic(output, self.thresh, self.num_classes, masked_anchors, len(self.anchor_mask),scale_x_y=self.scale_x_y,only_objectness=1,validation=False)
